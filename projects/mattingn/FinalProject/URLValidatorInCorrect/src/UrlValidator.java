...
public class UrlValidator implements Serializable {
    ...
    public UrlValidator(String[] schemes, RegexValidator authorityValidator, long options) {
        ...
        if (isOn(ALLOW_ALL_SCHEMES)) {
        	allowedSchemes = Collections.emptySet();// changed to empty set new HashSet<String>(0);
            ...
            allowedSchemes = new HashSet<String>(schemes.length); //change -1 to schemes.length
            
            for(int i=0; i < schemes.length; i++) { //change schemes.length+1 and removed the +1
            	allowedSchemes.add(schemes[i].toLowerCase(Locale.ENGLISH)); // changed i-1 to i
            }
        }
